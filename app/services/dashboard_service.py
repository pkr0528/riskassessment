...
def load_disk_models(
    ...
    clf_path=settings.SIGNAL_CLASSIFICATION_MODEL_PATH,
    scl_path=settings.STANDARD_SCALER_MODEL_PATH,
):
    reg = clf = scl = None
    try:
...
def render_dashboard(
    *,
    ...
    if results is None or results.empty:
        st.info(
            "No prediction results available. Run Tab 1 prediction or pass prediction_results."
        )
        return
    ...
    selected_section = st.selectbox(
        "Choose dashboard section to view", options=section_options, index=0
    )
    ...
    if show("Key Insights"):
        ...
        counts = (
            results[signal_col].value_counts().to_dict()
        ...
        mean_score = (
            results[score_col].mean() if score_col in results.columns else np.nan
        )

        c1.metric(" Green", f"{int(green)}")
        c2.metric(" Yellow", f"{int(yellow)}")
        c3.metric(" Red", f"{int(red)}")
        c4.metric(
            "Average Score", f"{mean_score:.2f}" if not np.isnan(mean_score) else "N/A"
        )
        st.markdown("---")
    ...
    if show("Visualizations"):
        ...
        with control_col:
            ...
            avail_signals = (
                sorted(results[signal_col].unique())
                if signal_col in results.columns
                else []
            )
            selected_signals = st.multiselect(
                "Signals", options=avail_signals, default=avail_signals
            )
        ...
        if score_col in viz_df.columns:
            viz_df = viz_df[
                (viz_df[score_col] >= score_range[0])
                & (viz_df[score_col] <= score_range[1])
            ]
        ...
        with viz_col:
            if score_col in viz_df.columns and signal_col in viz_df.columns:
                ...
                fig_box.update_layout(
                    title="Score distribution by Signal", yaxis_title=score_col
                )
    ...
    if show("Top Partners"):
        ...
        if "Final Score" in results.columns and "Final Signal" in results.columns:
            ...
            "Predicted Score" in results.columns
            and "Predicted Signal" in results.columns
        ):
            display_cols = ["Partner_Code", "Predicted Score", "Predicted Signal"]
            score_for_sort = "Predicted Score"
            st.warning(
                "Final Score/Signal not found — showing Predicted Score/Signal instead."
            )
        else:
            ...
            score_for_sort = next(
                (c for c in ["Final Score", "Predicted Score"] if c in results.columns),
                None,
            )
        ...
        if score_for_sort:
            top_risk = (
                results[
                    results[
                        display_cols[2] if len(display_cols) > 2 else display_cols[1]
                    ]
                    == "Red"
                ]
                .sort_values(score_for_sort)
                .head(top_n)
                if (
                    "Final Signal" in results.columns
                    or "Predicted Signal" in results.columns
                )
                else results.sort_values(score_for_sort).head(top_n)
            )
            top_safe = (
                results[
                    results[
                ...
                else results.sort_values(score_for_sort, ascending=False).head(top_n)
            )
        else:
        ...
        with c1:
            st.markdown("**Top Red**")
            st.dataframe(
                top_risk.reset_index(drop=True)[display_cols], use_container_width=True
            )
        with c2:
            st.markdown("**Top Green**")
            st.dataframe(
                top_safe.reset_index(drop=True)[display_cols], use_container_width=True
            )
        st.markdown("---")
